...
class Dataset(torch.utils.data.Dataset):
    ...
    def _discretization(self):
        ...
        if self.config["discretization"]:
            ...
            for field in dis_info.keys():
                if field not in self.field2type:
                    raise ValueError(f"Field [{field}] does not exist.")
                if field not in (self.config["numerical_features"] or []):
        ...
        for field in (self.config["numerical_features"] or []) or []:
    ...
    def num(self, field):
        ...
        if (
            ...
            and field in (self.config["numerical_features"] or [])
        ):
    ...
    def _dataframe_to_interaction(self, data):
        ...
        for k in data:
            ...
            if ftype == FeatureType.TOKEN:
                ...
                if k in (self.config["numerical_features"] or []):
