...
class AbstractDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if config["single_spec"] is False:
...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    @property
    def neg_dist(self):
        args = self.neg_sample_args or {}
        return args.get("distribution") or args.get("strategy", "none")

    def _set_neg_sample_args(self, config, dataset, dl_format, neg_sample_args):
        self.uid_field = dataset.uid_field
        self.iid_field = dataset.iid_field
        self.dl_format = dl_format
        self.neg_sample_args = neg_sample_args or {}
        self.times = 1
        dist = self.neg_sample_args.get("distribution") or self.neg_sample_args.get("strategy", "none")
        sample_num = self.neg_sample_args.get("sample_num", "none")
        if dist in ["uniform", "popularity"] and sample_num != "none":
            self.neg_sample_num = sample_num
        ...
        elif dist != "none" and sample_num != "none":
            raise ValueError(f"`neg_sample_args` [{dist}] is not supported!")
    ...
    def _neg_sampling(self, inter_feat):
        ...
        elif self.neg_dist != "none" and self.neg_sample_args.get("sample_num", "none") != "none":
            user_ids = inter_feat[self.uid_field].numpy()
            item_ids = inter_feat[self.iid_field].numpy()
