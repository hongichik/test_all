...
class NegSampleEvalDataLoader(NegSampleDataLoader):
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if (
            self.neg_dist != "none"
            and self.neg_sample_args.get("sample_num", "none") != "none"
        ):
...
class FullSortEvalDataLoader(AbstractDataLoader):
    ...
    def get_pos_len_list(self):
        return np.ones(self.sample_size, dtype=np.int64)

    def get_user_len_list(self):
        return np.full(self.sample_size, self._dataset.item_num, dtype=np.int64)
